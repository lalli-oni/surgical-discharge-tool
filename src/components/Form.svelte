<script lang="ts">
  ...
  import SingleChoice, { Choice } from "./inputs/SingleChoice.svelte";
  ...
  let organChoices = [{ id: "heart", label: labels.heart, icon: heartIcon, default: true }, { id: "lungs", label: labels.lungs, icon: lungIcon }];
  let rhythmsChoices: Array<Choice> = ["sinus", "chronicalAF", "newAF", "flutter"].map((r) => { return { id: r, label: labels.rhythms[r] } });
  let rhythmTreatmentChoices: Array<Choice> = ["metoprolol", "amiodarone", "defibrillation"].map((rt) => { return { id: rt, label: labels.rhythms.treatments[rt] } });
  let respiratoryChoices: Array<Choice> = ["noChanges"].map((r) => { return { id: r, label: labels.respiratory[r] } });
  let dischargeLocationChoices: Array<Choice> = labels.dischargeLocations.items;
  let validSelection = false;

  const originalSelection = {
    organ: organChoices[0],
    operationDate: null,
    rhythm: rhythmsChoices[0],
    rhythmTreatment: rhythmTreatmentChoices[0],
    respiratory: respiratoryChoices[0],
  ...
  export function resetSelection() {
    ...
    dispatch("selection-changed", null);
  };

  // Form validation
  $: if (selection.operationDate) validSelection = true;

  $: if (validSelection && selection) dispatch("selection-changed", selection);
</script>
...
<form class="grow min-w-max overflow-x-hidden overflow-y-auto">
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-2 md:gap-4">
    ...
    <section>
      ...
      <DatePicker
        bind:date={selection.operationDate}
    ...
    <section>
      ...
      <div class="flex flex-col gap-2">
        <SingleChoice
          choices={rhythmsChoices}
          bind:selected={selection.rhythm}
        />
      </div>
    </section>
    {#if selection.rhythm.id === "newAF" || selection.rhythm.id === "flutter"}
      <section in:fly="{{ y: -100, duration: 800 }}" out:fade={{ duration: 300 }}>
        <h2>{labels.rhythms.treatments.label}</h2>
        <div class="flex flex-col gap-2">
          <SingleChoice
            choices={rhythmTreatmentChoices}
            bind:selected={selection.rhythmTreatment}
          />
        </div>
      </section>
    {/if}
    <section>
      <h2>{labels.respiratory.label}</h2>
      <SingleChoice
        choices={respiratoryChoices}
        bind:selected={selection.respiratory}
      />
    </section>
    <section>
